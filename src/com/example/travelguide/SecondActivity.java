...
public class SecondActivity extends Activity implements LocationSource,
		...
		OnMapLoadedListener, OnClickListener, TextWatcher, InfoWindowAdapter,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (area == 4)
			numb1 = bundle.getInt("number5"); // 获取Intent的内容age

		if (area == 4)
			numb2 = bundle.getInt("number6"); // 获取Intent的内容age
		...
		if (numb1 != 0 && numb2 != 0) {
			...
			if (numb1 == 1) {
				...
				if (area == 4)
					mm = bundle.getDouble("number7"); // 获取Intent的内容age
				if (area == 4)
					nn = bundle.getDouble("number8"); // 获取Intent的内容age
				m1 = mm;
				m2 = nn;
		...
		btn2.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				mLocationManagerProxy.requestLocationData(
						LocationProviderProxy.AMapNetwork, -1, 3,
						SecondActivity.this);
	...
	public void onPoiSearched(PoiResult result, int rCode) {
		...
		if (rCode == 0) {
			if (result != null && result.getQuery() != null) {// 搜索poi的结果
				if (result.getQuery().equals(query)) {// 是否是同一条
					...
					if (poiItems != null && poiItems.size() > 0) {
						...
						ToastUtil.show(SecondActivity.this, R.string.no_result);
					}
				}
			} else {
				ToastUtil.show(SecondActivity.this, R.string.no_result);
			...
			ToastUtil.show(SecondActivity.this, R.string.error_network);
		} else if (rCode == 32) {
			ToastUtil.show(SecondActivity.this, R.string.error_key);
		} else {
			ToastUtil.show(SecondActivity.this, getString(R.string.error_other)
					+ rCode);
		}
	...
	private void addMarkersToMap() {
		...
		// intent_broadcast[1].setClass(SecondActivity.this,
		// MusicActivity.class);
		// }
		//
		// dis[1][1]=26.05124;
		// dis[1][2]=119.19283;intent_broadcast[1].putExtra("num",1);intent_broadcast[1].setClass(SecondActivity.this,
		// MusicActivity.class);//电脑之家
		...
		latLng = new LatLng(26.05124, 119.19283);// 电脑之家
	...
	public void addMarker(LatLng a, String title, String picture, float s1,
			float s2) {
		aMap.addMarker(new MarkerOptions().anchor(s1, s2)
				.icon(BitmapDescriptorFactory.fromAsset(picture)).position(a)
				.title(title).setFlat(true).draggable(true));
	}
	...
	public void onMapLoaded() {
		...
		LatLngBounds bounds = new LatLngBounds.Builder().include(northeast)
				.include(mar1).include(southwest).build();
	}

	public boolean onMarkerClick(final Marker marker) {
		if (marker.getTitle().equals("电脑之家"))
			markerText.setText("这里是" + marker.getTitle()
					+ "经营一些电脑有关的产品，比如鼠标，以及维修电脑。");
	...
	public void onLocationChanged(AMapLocation amapLocation) {
		if (mListener != null && amapLocation != null) {
			if (amapLocation.getAMapException().getErrorCode() == 0) {
				updateLocation(amapLocation.getLatitude(),
				...
				LatLng Fudathree = new LatLng(26.051554, 119.192267);
				LatLng Fudafour = new LatLng(26.050508, 119.189971);
				// addMarker(26.050908,119.19187,"千艺美发","1.png");//
				...
				if (area == 3)// 移动到中心区域
				{
					if (AMapUtils.calculateLineDistance(latLng, D3) <= 100) {
				...
				LatLng endLatlng=new LatLng(26.051106,119.192433);
           	 if(AMapUtils.calculateLineDistance(latLng, endLatlng)<=22){
